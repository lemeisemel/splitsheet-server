...
app.get('/callback', async (req, res) => {
  ...
  try {
    ...
    res.cookie('splitsheet_session', sign(robloxUserId), {
      httpOnly: true,
      secure: true,
      sameSite: 'none', // needed since the site and API are on different domains
      maxAge: 1000 * 60 * 60 * 24 * 90, // 90 days
    });

    // Simple confirmation page the popup/tab can close, or redirect back to the tool.
    res.send(`
      <!DOCTYPE html>
      <html><body style="font-family: sans-serif; text-align:center; padding-top:60px;">
        <h2>Connected as ${userInfo.nickname || 'your Roblox account'} ✓</h2>
        <p>You can close this tab and go back to Splitsheet.</p>
        <script>
          if (window.opener) { window.opener.postMessage('splitsheet-auth-complete', '*'); }
        </script>
      </body></html>
    `);
  } catch (err) {
...
app.get('/me', async (req, res) => {
  const userId = unsign(req.cookies.splitsheet_session);
  ...
  if (error || !data) return res.json({ loggedIn: false });
  res.json({ loggedIn: true, username: data.username, userId: data.roblox_user_id });
});
